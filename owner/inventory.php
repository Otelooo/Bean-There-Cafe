...
<nav id="sidebar">
  ...
  <a href="inventory.php" class="nav-item active"><i class="fas fa-boxes-stacked"></i> Inventory <span class="nav-badge">3</span></a>
  ...
  <div class="nav-item" onclick="showToast('Settings — coming soon!','success')"><i class="fas fa-gear"></i> System Settings</div>
...
function renderInventory(search, cat, level) {
  ...
  tbody.innerHTML = filtered.map(i => {
    // Check if the category is Supplies or Pastries; otherwise, output an em dash
    const displayContact = (i.cat === 'Supplies' || i.cat === 'Pastries') ? i.contact : '—';
    ...
    return `
      <tr>
        ...
        <td>${i.supplier}</td>
        <td class="text-mono" style="font-size:12px;">${displayContact}</td>
