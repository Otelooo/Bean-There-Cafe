...
<nav id="sidebar">
  ...
  <a href="order_queue.php" class="nav-item active"><i class="fas fa-list-check"></i> Order Queue</a>
    ...
    <div class="table-wrap">
      <table class="data-table">
        ...
        <tbody id="order-tbody">
          ...
          <?php else: ?>
            <?php $idx = 1; foreach ($orders as $tid => $order): ?>
              <?php
...
<script>
  ...
  async function updateOrderStatus(select) {
    ...
    try {
      const res = await fetch('order_queue.php', {
        method: 'POST',
