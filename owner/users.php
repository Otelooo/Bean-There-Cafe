...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if ($newUsername === '' || $newPassword === '' || !in_array($newRole, ['cafe owner', 'cafe staff'], true)) {
            ...
            if ($exists) {
                ...
                $insert = $conn->prepare("INSERT INTO users (username, password, role, status) VALUES (?, ?, ?, 'active')");
                $insert->bind_param('sss', $newUsername, $hashed, $newRole);
                $insert->execute();
                $insert->close();
        ...
        if ($editId <= 0 || $editUsername === '' || !in_array($editRole, ['cafe owner', 'cafe staff'], true)) {
            $msg = 'Please fill in all fields correctly.';
            $msgType = 'warn';
        } else {
            $check = $conn->prepare('SELECT user_id FROM users WHERE username = ? AND user_id != ?');
            $check->bind_param('si', $editUsername, $editId);
            $check->execute();
            $exists = $check->get_result()->fetch_assoc();
            $check->close();
            ...
            if ($exists) {
                ...
                $hashed = password_hash($editPassword, PASSWORD_DEFAULT);
                $update = $conn->prepare('UPDATE users SET username = ?, role = ?, password = ? WHERE user_id = ?');
                $update->bind_param('sssi', $editUsername, $editRole, $hashed, $editId);
                $update->execute();
                $update->close();
                $msg = 'Account updated successfully.';
            } else {
                $update = $conn->prepare('UPDATE users SET username = ?, role = ? WHERE user_id = ?');
                $update->bind_param('ssi', $editUsername, $editRole, $editId);
                $update->execute();
                $update->close();
                $msg = 'Account updated successfully.';
            }
        ...
        if (!in_array($newStatus, ['active', 'inactive'], true)) {
            ...
            $update->execute();
            $update->close();
            $msg = $newStatus === 'inactive' ? 'Account deactivated.' : 'Account activated.';
        }
    }

    header('Location: users.php?msg=' . urlencode($msg) . '&type=' . urlencode($msgType));
    exit;
}
...
<head>
  ...
  <title>User Management | SmartStock</title>
...
<body>
  ...
  <div id="main">
    ...
    <div class="page-strip">
      ...
      <button class="btn-primary" onclick="openModal('modal-add-user')"><i class="fas fa-user-plus"></i> Add
        User</button>
    </div>
    <div style="padding:22px 26px;">
      <div class="inv-toolbar">
        <div class="search-box"><i class="fas fa-magnifying-glass"></i><input type="text" placeholder="Search users…"
            oninput="filterUsers(this.value)" /></div>
        <select class="filter-select" onchange="showToast('Filter applied','success')">
        ...
        <table class="data-table" id="user-table">
          ...
          <tbody id="user-tbody">
            <?php foreach ($users as $u): ?>
              <?php
                $roleLabel = $u['role'] === 'cafe owner' ? 'Owner' : 'Staff';
                $rolePillClass = $u['role'] === 'cafe owner' ? 'pill-gold' : 'pill-success';
                $statusLabel = $u['status'] === 'active' ? 'Active' : 'Inactive';
                $statusPillClass = $u['status'] === 'active' ? 'pill-success' : 'pill-red';
                $nextStatus = $u['status'] === 'active' ? 'inactive' : 'active';
                $toggleLabel = $u['status'] === 'active' ? 'Deactivate' : 'Activate';
                $toggleClass = $u['status'] === 'active' ? 'tbl-btn-del' : 'tbl-btn-act';
              ?>
              <tr>
  ...
  <div class="modal-overlay" id="modal-add-user">
    <div class="modal-box">
      <div class="modal-title"><i class="fas fa-user-plus" style="color:var(--gold);margin-right:8px;"></i>Add Staff
        Account</div>
      <div class="modal-sub">Create a new user account.</div>
      <form method="POST" action="users.php">
        <input type="hidden" name="action" value="add">
        <div class="modal-field"><label>Username</label><input type="text" name="username"
            placeholder="firstname.lastname" required /></div>
        <div class="modal-field"><label>Role</label><select name="role">
            <option value="cafe staff">Staff</option>
            <option value="cafe owner">Owner</option>
          </select></div>
        <div class="modal-field"><label>Password</label><input type="password" name="password"
            placeholder="Min. 8 characters" required /></div>
        <button type="submit" class="btn-modal-primary"><i class="fas fa-check"
            style="margin-right:6px;"></i>Create Account</button>
        <button type="button" class="btn-modal-cancel" onclick="closeModal('modal-add-user')">Cancel</button>
      </form>
    </div>
  </div>
  ...
  <div class="modal-overlay" id="modal-edit-user">
    <div class="modal-box">
      <div class="modal-title"><i class="fas fa-user-pen" style="color:var(--gold);margin-right:8px;"></i>Edit Staff
        Account</div>
      <div class="modal-sub">Update account details.</div>
      <form method="POST" action="users.php">
        <input type="hidden" name="action" value="edit">
        <input type="hidden" name="user_id" id="edit-user-id" value="" />
        <div class="modal-field"><label>Username</label><input type="text" name="username" id="edit-username"
            required /></div>
        <div class="modal-field"><label>Role</label><select name="role" id="edit-role">
            <option value="cafe staff">Staff</option>
            <option value="cafe owner">Owner</option>
          </select></div>
        <div class="modal-field"><label>New Password</label><input type="password" name="password"
            placeholder="Leave blank to keep current password" /></div>
        <button type="submit" class="btn-modal-primary"><i class="fas fa-check"
            style="margin-right:6px;"></i>Save Changes</button>
        <button type="button" class="btn-modal-cancel" onclick="closeModal('modal-edit-user')">Cancel</button>
      </form>
    </div>
  </div>
